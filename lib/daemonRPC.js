...
const axios = require('axios'); // TODO eliminate external dependency
...
class daemonRPC {
   ...
   *
   * @function relay_tx
   * @param {array} txids - height number or hash
   *
   * @returns {promise} resolve, reject
   * @resolves {object} - Example: {
   *   x: ''
  ...
  relay_tx(txids) {
    ...
    return this._run('relay_tx', params);
  }

  /**
   * Get the daemons peer list to find other public nodes
   * 
   * @function get_public_nodes
   * @param {boolean} gray - include gray nodes (offline nodes, default: false)
   * @param {boolean} white - include white nodes (online nodes, default: true)
   * 
   * @returns {promise} resolve, reject
   * @resolves {object} - Example: { 
   * status: 'OK',
   ...
   * [ { 
   *      host: '51.81.35.134',
   *      last_seen: 1614690799,
   *      rpc_credits_per_hash: 0,
   *      rpc_port: 18088,
   * }]
   * }
   */
  get_public_nodes(gray = false, white = true) {
    let params = {
      gray: gray,
      white: white
    };

    return this._run(null, params, 'get_public_nodes');
  }

  /**
   * Clear transactions from the daemon cache
   * 
   * @function flush_cache
   * @param {boolean} bad_txs - clear cache storing transactions that failed verification
   * @param {boolean} bad_blocks - clear cache storing blocks that failed verification
   * 
   * @returns {promise} resolve, reject
   * @resolves {object} - Example {
   * status: 'OK', 
   ...
   * }
   */
  flush_cache(bad_txs = true, bad_blocks = true) {
    let params = {
      bad_txs: bad_txs,
      bad_blocks: bad_blocks
    };

    return this._run('flush_cache', params)
  }

  /**
   * Bootstrap a remote daemon that the connected wallets can use while this daemon synchronizes 
   * 
   * @function set_bootstrap_daemon
   * @param {string} url - URL of a 'bootstrap' remote daemon
   * @param {string} username - username to use if remote daemon requires credentials (Optional)
   * @param {string} password - password for username if remote daemon required credentials (Optional)
   * 
   * @returns {promise} resolve, reject
   * @resolves {object} - Example {
   * status: 'OK'
   * }
   */
  set_bootstrap_daemon(url, username = '', password = '') {
    let params = {
      url: url,
      username: username,
      password: password,
    ...
    return this._run(null, params, 'set_bootstrap_daemon')
  }
}
