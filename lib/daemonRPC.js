...
var request = require('request-promise'); // TODO eliminate external dependency
...
class daemonRPC {
  constructor(hostname = '127.0.0.1', port = 18081, user = undefined, pass = undefined, protocol = 'http', network = 'mainnet') {
    return new Promise((resolve, reject) => {
      ...
      if (typeof hostname == 'object') { // parameters can be passed in as object/dictionary
        ...
        if (network in this.daemons) {
          this.daemons = this.daemons[network];
        } else {
          throw new Error(`No ${network} remote daemons found in list`);
        ...
        if ('autoconnect' in params) {
          if (params['autoconnect']) { // TODO add mainnet/testnet toggle
            ...
            if (network in daemonsLocal) {
              daemonsLocal = daemonsLocal[network];
            } else {
              throw new Error(`No ${network} local daemons found in list`);
  ...
  _autoconnect(daemons = this.daemons) {
    return new Promise((resolve, reject) => {
      if (this.network) {
        if (this.network in daemons) {
          daemons = daemons[this.network];  
        } else {
          throw new Error(`No ${network} daemons found in list`);
          daemons = daemons['mainnet'];
        }
      } else {
        daemons = daemons['mainnet'];
      }
   ...
   */
  getheight() {
    return this._run(null, undefined, 'getheight');
  }
  ...
  get_alt_blocks_hashes() {
    return this._run(null, undefined, 'get_alt_blocks_hashes');
  }
  ...
  is_key_image_spent(key_images) {
    let params = { key_images: key_images };
   ...
   */
  stop_daemon() {
    return this._run(null, undefined, 'stop_daemon');
  }
   ...
   */
  get_limit() {
    return this._run(null, undefined, 'get_limit');
  }
   ...
   */
  stop_mining() {
    return this._run(null, undefined, 'stop_mining');
  }
   ...
   */
  mining_status() {
    return this._run(null, undefined, 'mining_status');
  }
   ...
   */
  save_bc() {
    return this._run(null, undefined, 'save_bc');
  }
   ...
   */
  get_peer_list() {
    return this._run(null, undefined, 'get_peer_list');
  }
   ...
   * @param {numbber} visible - Set log verbosity from 0 (less verbose) to 4 (most verbose)
   *
   * @returns {string} JSON string - Example: {
   *   status: "OK"
   * }
  ...
  get_transaction_pool() {
    return this._run(null, undefined, 'get_transaction_pool');
  }
  ...
  get_transaction_pool_stats() {
    return this._run(null, undefined, 'get_transaction_pool_stats');
  }
