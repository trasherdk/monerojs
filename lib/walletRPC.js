...
class walletRPC {
  ...
  /**
   * Adds entries to the wallets address book
   *
   ...
   * @param {string} filename - Filename to use for the wallet to close (default: 'monero_wallet')
   *
   * @returns {promise} resolve, reject
   * @resolves {object} - Example: {}
   */
  close_wallet(filename = 'monero_wallet') {
    let params = {
      filename: filename
    };

    return this._run('close_wallet', params);
  }
  ...
  get_version() {
    return this._run('get_version');
     ...
     /**
     * Create and open a wallet on RPC Server from existing mnemonic phrase (closes any open wallet in RPC server)
     * 
     * @function restore_seed
     * @param {string} name - name of wallet on RPC server
     * @param {string} password - password to wallet file
     * @param {string} seed - mnemonic phrase of the wallet to restore
     * @param {integer} restore_height - height to restore wallet from (Optional, default: 0)
     * @param {string} language - wallet language (Optional)
     * @param {string} seed_offset - offset used to derive a new seed from the given mnemonic to recover a secret wallet from the mnemonic phrase (Optional)
     * @param {boolean} autosave_current - save the currently open RPC wallet before closing (default: true)
     * 
     * @returns {promise} resolve, reject
     * @resolves {object} - Example: { 
     * address: '46MQi4JGpuPSr6WXNFh17k4WQWnkkoVZdiAUE1DtSZA4bgwEBtsWFUKG2AKKDSzGh1B8P8s5hC2GLYkeV3SvhgjCGekhtPy',
     * info: 'Wallet has been restored successfully.',
     * seed: 'dauntless tutor brunt sieve army wedge spud uneven biology podcast soothe vinegar rash birth upgrade upstairs optical avoid awoken pledge saucepan shuffled jellyfish lesson biology',
     * was_deprecated: false 
     * }
     */
    restore_deterministic_wallet(name, password, seed, restore_height = 0, language, seed_offset, autosave_current=true) {
      let params = {
        name: name,
        password: password,
        seed: seed,
    ...
    /**
     * Alias of restore_deterministic_wallet
     */
    restore_seed(name, password, seed, restore_height = 0, language, seed_offset, autosave_current=true) {
      let params = {
        name: name,
        password: password,
        seed: seed,
  ...
  /**
   * Set daemon to connect RPC wallet to
   *
   * @function set_daemon
   * @param {string} address - URL of daemon to connect to
   * @param {boolean} trusted - if false, some RPC wallet methods will be disabled (default: false)
   * @param {string} ssl_support - specifies whether the daemon uses SSL encryption (default: autodetect, options: disabled, enabled, autodetect)
   * @param {string} ssl_private_key_path - file path location of the SSL key (Optional)
   * @param {string} ssl_certificate_path - The file path location of the SSL certificate. (Optional)
   * @param {string} ssl_ca_file - The file path location of the certificate authority file. (Optional)
   * @param {array} ssl_allowed_fingerprints - The SHA1 fingerprints accepted by the SSL certificate. (Optional)
   * @param {boolean} ssl_allow_any_cert - If false, the certificate must be signed by a trusted certificate authority (Optional, default: false)
   *
   * @returns {promise} resolve, reject
   * @resolves {object} - Example: {}
  */
  set_daemon(address, trusted = false, ssl_support = 'autodetect', ssl_private_key_path, ssl_certificate_path, ssl_ca_file, ssl_allowed_fingerprints, ssl_allow_any_cert) {
    let params = {
      address: address,
      trusted: trusted,
    ...
    return this._run('set_daemon', params);
  }

  /**
   * Validate an address to determine whether it is a valid address and returns the result and the address specifications.
   * 
   * @function validate_address
   * @param {string} address - the address to validate
   * @param {boolean} any_net_type - if true, checks all available networks (mainnet, stagenet, testnet). Otherwise, only check using the network wallet-rpc is running. (Optional, default: false)
   * @param {boolean} allow_openalias - if true, consider OpenAlias-formatted address valid (Optional, default: false)
   * 
   * @returns {promise} resolve, reject
   * @resolves {object} - Example: { 
   * integrated: false,
  ...
  /**
   * 
   * NOTE: As of March 01, 2021, usinge restore from keys will not allow you to retrieve mnemonic seed
   * as the seed language is not set in this RPC. An issue is open with the Monero Project regarding
   * this. Once resolved, this message will be removed.
   * 
   * Restores a wallet from a given wallet address, view key, and optional spend key.
   * 
   * @function generate_from_keys
   * @param {integer} restore_height - Height to restore from (Optional, default: 0)
   * @param {string} filename - wallet file name on RPC server
   * @param {string} address - wallet primary address 
   * @param {string} spendkey - wallet private spend key
   * @param {string} viewkey - wallet view key
   * @param {string} password - wallet password 
   * @param {boolean} autosave_current - Save the wallet before generating (default: true)
   * @param {string} language - Wallet language
   * 
   * @returns {promise} resolve, reject
   * @resolves {object} - Example: {
   *  id": "0",
   ...
   * } 
   */
  generate_from_keys(restore_height, filename, address, spendkey, viewkey, password, autosave_current = true, language = 'English') {
    let params = {
      restore_height: restore_height,
      filename: filename,
      address: address,
 ...
 * @param {boolean} enabled - If true, automatically refresh wallet after _period_ of time. (Optional, default: true)
 * @param {unsigned integer} period - length of time in seconds to wait before refreshing. (Optional)
 * 
 * @returns {promise} resolve, reject
 * @resolves {object} - Example: {}
 */
  auto_refresh(enabled = true, period) {
    let params = {
      enable: enabled,
      period: period
    };
  ...
  /** 
   * Edits an entry in the open wallets address book.
   * 
   * @function edit_address_book
   * @param {unsigned integer} index - index of address book entry to edit.
   * @param {boolean} set_address - If true, set the address for this entry to the value of "address". (default: true)
   * @param {string} address - the public address to add to the book. 
   * @param {boolean} set_description - If true, set the description for this entry to the value of "description". (Optional, default: false)
   * @param {string} description - Human-readable description for this entry. (Optional)
   * @param {boolean} set_payment_id - If true, set the payment ID for this entry to the value of "payment_id". (Optional, default: false)
   * @param {string} payment_id - Payment ID for this address. (Optional)
   * 
   * @returns {promise} resolve, reject
   * @resolves {object} - Example: {}
   */
  edit_address_book(index, set_address = true, address, set_description = false, description, set_payment_id = false, payment_id) {
    return new Promise((resolve, reject) => {
      let params = {
        index: index,
      ...
      this._run('edit_address_book', params)
        .then((result) => {
          this.store() // Save wallet state after editting address book
            .then(() => {
              resolve(result);
            })
            .catch(err => {
