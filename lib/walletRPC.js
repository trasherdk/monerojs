...
class walletRPC {
   ...
   * }
   */
  sweep_single(key_image, address, payment_id = undefined, get_tx_keys = false, mixin = 6, priority = 2, below_amount = 0, unlock_time = undefined, do_not_relay = 0, get_tx_hex = false, get_tx_metadata = false) {
   ...
   * @param {number} account_minreserve - Do not prove reserves of subaddress within account if it holds less than this amount                     (optional)
   *
   * @returns   Example: {
   ...
   * }
   */
  get_reserve_proof(account_index = 'all', account_minreserve = undefined) {
    if (account_index == 'all') {
      let params = {
        all: true,
        account_minreserve: account_minreserve
      };
    } else {
      let params = {
        account_index: account_index,
