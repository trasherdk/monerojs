...
class walletRPC {
  constructor(hostname = '127.0.0.1', port = 28083, user = undefined, pass = undefined, protocol = 'http', network = 'mainnet') {
    return new Promise((resolve, reject) => {
      ...
      if (typeof hostname == 'object') { // Parameters can be passed in as object/dictionary
        ...
        if (network in this.wallets) {
          this.wallets = this.wallets[network];
        } else {
          throw new Error(`No ${network} wallets found in list`);
  ...
  _autoconnect(wallets = this.wallets) {
    return new Promise((resolve, reject) => {
      if (this.network) {
        if (this.network in wallets) {
          wallets = wallets[this.network];  
        } else {
          throw new Error(`No ${network} wallets found in list`);
          wallets = wallets['mainnet'];
        }
      } else {
        wallets = wallets['mainnet'];
      }
      ...
      .catch((error) => {
        if (wallets.length > 0) {
          ...
          reject('Failed to autoconnect to wallet');
        }
      });
    });
  }
